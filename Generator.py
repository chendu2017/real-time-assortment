...
class Customer():
    ...
    def Choose_Product(self,provided_set,seller,chooseType):
        ...
        if  chooseType == 1 :                     #--- 按概率大小选择
            customer_choice = np.random.choice(range(0,10),probability)
             
        elif  chooseType == 2:                    #--- 选preference最大
            customer_choice = probability.index(max(probability))

        elif  chooseType == 3:                    #--- 选preference最小
            customer_choice = probability.index(min(probability))
